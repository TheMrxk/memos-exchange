    ...
    register(api) {
        ...
        api.on('agent_end', async (event, ctx) => {
            ...
            try {
                ...
                // 确定用于生成记忆的内容
                // 优先使用缓存的用户原始消息，如果没有则使用 assistant 回复
                let userContentForMemory = cachedUserPrompt;
                if (!userContentForMemory || userContentForMemory.length < 5) {
                    // 使用 assistant 回复作为记忆内容（包含上下文信息）
                    if (assistant && assistant.length > 10) {
                        userContentForMemory = '[助手回复] ' + assistant.substring(0, 500);
                        fileLog('使用助手回复作为记忆内容');
                    }
                }
                ...
                log.info('[memory-exchange] 准备保存对话', { user: user.substring(0, 50), assistant: assistant?.substring(0, 50) });
                fileLog('准备保存对话');

                // 保存到数据库
                const convId = saveConversation(cfg, sessionKey, user, assistant);
